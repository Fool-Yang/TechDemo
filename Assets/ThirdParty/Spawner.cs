...
namespace UnityMovementAI
{
    public class Spawner : MonoBehaviour
    {
        ...
        public MovementAIRigidbody[] thingsToBeAvoided;
        ...
        bool CanPlaceObject(float halfSize, Vector3 pos)
        {
            ...
            for (int i = 0; i < thingsToBeAvoided.Length; i++)
            {
                float dist = Vector3.Distance(thingsToBeAvoided[i].Position, pos);

                if (dist < halfSize + thingsToBeAvoided[i].Radius)
                {
                    return false;
                }
            }
